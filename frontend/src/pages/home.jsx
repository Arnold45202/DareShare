...
function Postify(post) {
  ...
  const [likesColor, setLikesColor] = useState('white');
  const [comments, setComments] = useState(post.comments);
  const [commentsColor, setCommentsColor] = useState('white');
  const [shares, setShares] = useState(post.shares);
  const [sharesColor, setSharesColor] = useState('white');
  ...
  return (
    <div className='post-grid'>
      ...
      <div className='social-buttons'>
        <div className='social-button-info'>
          <div className='icon'>
            <FavoriteIcon
              sx={{ color: likesColor }}
              onClick={() => {
                likesColor === 'red' ? setLikesColor('white') : setLikesColor('red');
                likesColor === 'red' ? setLikes(likes - 1) : setLikes(likes + 1);
              }}
            />
          </div>
          {likes}
        ...
        <div className='social-button-info'>
          <div className='icon'>
            <MessageIcon
              sx={{ color: commentsColor }}
              onClick={() => {
                commentsColor === 'blue' ? setCommentsColor('white') : setCommentsColor('blue');
                commentsColor === 'blue' ? setComments(comments - 1) : setComments(comments + 1);
              }}
            />
          </div>
          {comments}
        ...
        <div className='social-button-info'>
          <div className='icon'>
            <ShareIcon
              sx={{ color: sharesColor }}
              onClick={() => {
                sharesColor === 'green' ? setSharesColor('white') : setSharesColor('green');
								sharesColor === 'green' ? setShares(shares - 1) : setShares(shares + 1);
              }}
            />
          </div>
          {shares}
...
function Home() {
  ...
  return (
    <div className='container'>
      ...
      <div className='posts-container'>{posts.map((post) => Postify(post))}</div>
